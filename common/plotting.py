...
def get_topology(edges):
    ...
    for row in edges.itertuples():
        src = row.Interface["hostname"] if isinstance(row.Interface, dict) else row.Interface.hostname
        dst = row.Remote_Interface["hostname"] if isinstance(row.Remote_Interface, dict) else row.Remote_Interface.hostname
        
        src_iface = row.Interface.get("interface", "") if isinstance(row.Interface, dict) else getattr(row.Interface, "interface", "")
        dst_iface = row.Remote_Interface.get("interface", "") if isinstance(row.Remote_Interface, dict) else getattr(row.Remote_Interface, "interface", "")
